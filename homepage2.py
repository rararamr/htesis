...
def create_new_window2(parent):
    ...
    home.geometry("800x600")  # Initial size
    ...
    def resize_image(event=None):
        ...
        new_width = home.winfo_width()
        new_height = home.winfo_height()
        ...
        adjusted_img = enhancer.enhance(0.5)

        # Convert the resized image to a PhotoImage
        new_img_tk = ImageTk.PhotoImage(adjusted_img)
    ...
    l2 = customtkinter.CTkLabel(home)
    l2.pack(fill='both', expand=True)  # Make sure it covers the entire window
    resize_image()  # Initial call to set the image
    ...
    frame = customtkinter.CTkFrame(master=l2, width=950, height=550, corner_radius=15)
    ...
    label2 = customtkinter.CTkLabel(master=frame, text="CONTROL PANEL", font=('Century Gothic', 30))
    ...
    def resize_image(event):
        ...
        if new_width > 300 and new_height > 300:
            ...
            # Enhance and adjust the image's brightness
            enhancer = ImageEnhance.Brightness(new_img)
            adjusted_img = enhancer.enhance(0.5)  # Keep the brightness consistent
    ...
    def initialize_window(home):
        # Set a minimum size for the window to prevent it from being too small
        home.geometry("800x600")  # Set initial window size
        home.minsize(800, 600)  # Set minimum window size
        
    # Initialize the window with minimum size
    initialize_window(home)
    def display_df(df, master):
        # Ensure the window resizes dynamically to fit the content
        def resize_window():
            # Automatically resize based on treeview size
            tree_width = sum([tree.column(c, width=None) for c in tree["columns"]])
            new_width = max(800, tree_width + 40)  # Ensure a minimum width of 800
            new_height = 600 + 200  # Height with space for buttons and table
        ...
        # Add buttons inside the button_frame
        remove_button = customtkinter.CTkButton(button_frame, text="Remove CSV", command=remove_csv)
        upload_button = customtkinter.CTkButton(button_frame, text="Upload CSV", command=upload_csv)
        analyze_button = customtkinter.CTkButton(button_frame, text="Analyze Ratings", command=analyze_ratings)
        exit_button = customtkinter.CTkButton(button_frame, text="Exit", command=exit_program)
        view_button = customtkinter.CTkButton(button_frame, text="View CSV", command=toggle_table)

        # Pack buttons in the frame
        remove_button.pack(padx=10, pady=10, fill=tk.X)
        upload_button.pack(padx=10, pady=10, fill=tk.X)
        analyze_button.pack(padx=10, pady=10, fill=tk.X)
        view_button.pack(padx=10, pady=10, fill=tk.X)  # View CSV button is now aligned with other buttons
        exit_button.pack(padx=10, pady=10, fill=tk.X)

        y_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        x_scrollbar.pack(side=tk.BOTTOM, fill=tk.X)
        
        # Return the frame containing buttons and table
        return table_frame
    ...
    remove_button = customtkinter.CTkButton(master=frame, text="Remove CSV", fg_color="#787276", command=remove_csv)  
    remove_button.place(x=10, y=350)
    # Create the "Analyze Ratings" button
    analyze_button = customtkinter.CTkButton(master=frame, text="Analyze Ratings", fg_color="#FFD700", command=analyze_ratings)
    analyze_button.configure(text_color="black")
    analyze_button.place(x=10, y=400)

    exit_button = customtkinter.CTkButton(master=frame, text="Exit", command=exit_program, fg_color="red", text_color="white", hover_color="#FF0000")
    exit_button.place(x=10, y=450)
    
    settings_button = customtkinter.CTkButton(master=frame, text="Settings", command=settings, fg_color="green", text_color="white", hover_color="#FF0000")
